...
class model_checkpoint(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch: int, logs: dict = None) -> None:
        ...
        if self.sub:
            temp_path = f"{self.name}_sub_diagnostic_weights.weights.h5" # Here we are saving the sub_diagnostic model weights when we run (train.py with --sub=True) for visualisation
        else:
            temp_path = f"{self.name}_weights.weights.h5" # Here we are saving the model weights when we run train.py
